...
class Item(object):
    ...
    def write_to_file(self):
        # if int(self.price) < 600:
            # self.comment = "Very low Price : Probable Scam"
...
def main_head_locator(link):
    ...
    source_code = source_code.split('<div class="open-map-view-button">')[-1]
    ...
    for head in head1_name_list:
        ...
        if "result-title hdrlnk" in head:
            ...
            url_link = head.split('"')[1]
            name = (head.split('>')[1]).split("<")[0]
            if "," in name:
                name = name.replace(",", "")
            try:
                pre_price = head.split('result-price">')[1]
                price = int((pre_price.split('$')[1]).split("<")[0])
            except:
                break
            temp = Item()
            temp.name = name
...
mp4_dict = dict()
url = 'https://santabarbara.craigslist.org/d/rooms-shares/search/roo'
